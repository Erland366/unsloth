#!/usr/bin/env python3
"""
GLM 4.7 Flash Benchmark Script (Unsloth)

Measures VRAM usage and throughput for GLM 4.7 Flash fine-tuning with Unsloth.
...
def run_unsloth_benchmark(config: BenchmarkConfig) -> BenchmarkResult:
    """Run the Unsloth benchmark with the given configuration."""
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        ...
        unsloth_force_compile=True,  # Enable to allow FA2 with trust_remote_code
    ...
    # Unsloth auto-detects MoE and adds expert LoRA via target_parameters
    model = FastLanguageModel.get_peft_model(
        ...
        target_modules=[
            ...
            "o_proj",
            # Include gate_up_proj to trigger MoE expert LoRA detection
...
run_benchmark = run_unsloth_benchmark


def parse_args():
    parser = argparse.ArgumentParser(description="Benchmark GLM 4.7 Flash fine-tuning with Unsloth")
...
def main():
    ...
    print("=" * 70)
    print("GLM 4.7 Flash Benchmark (Unsloth)")
    print("=" * 70)
    print(f"Config:")
    ...
    result = run_unsloth_benchmark(config)
    print(result)
